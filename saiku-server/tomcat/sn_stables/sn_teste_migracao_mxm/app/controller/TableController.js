...
//centro_de_custo_mtz
		...
		if(ano == $scope.anoComparativo)
			{
				...
				return 'Comparativo Orc '+($scope.anoComparativo-1)+' c/ Real '+($scope.anoComparativo-2);
			}
		else
			return ano;
	};
	...
	$scope.anoAtual = new Date().getFullYear();//2014
	$scope.hojeMes = $scope.mesAtual = new Date().getMonth();// 12
	...
	{
		var v2 = v*100;
		if(s && (s.indexOf('RECEITA') > -1 || s.indexOf('DESP. CORPORATIVA') >-1 ))
		{
			v2 = v2 * -1;
		}
		var g = new String(v2).match(/\-*\d+\.*\d{1}/);		
		var x = (g ? g[0] : "0")+" %";
		return x;
	};
