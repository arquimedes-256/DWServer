...
var Table = Backbone.View.extend({
    ...
    process_data: function(data) {
        ...
        contents += this.insertFooter(data);
    ...
    insertFooter: function(data)
    {
    	var contents = "";
    	/*if(AdvancedStsController.measuresMap.length > 0)
 		{	
         	contents += '<tr><th class="row_header"><div>Somat&oacute;rio (&Sigma;)</div></th>';
 		}
        for ( var u = 0; u < AdvancedStsController.measuresMap[0].col; u++) 
        {
        	 contents+='<td class="data">-</td>';
		}*/
    	contents += this.renderCalc('Somat&oacute;rio (&Sigma;)','sum');
    	contents += this.renderCalc('M&eacute;dia (&#955;)','avg');
    	contents += this.renderCalc('M&eacute;dia (%)','avgPercent','%');
    	contents += this.renderCalc('Contagem (&#951)','count',false);
    	return contents;
    },
    renderCalc:function(name,prop,s)
    {
    	var contents = '';
    	if(AdvancedStsController.getMeasuresDimension() == DimensionType.COLUMNS)
    		{
	    		contents = '<tr><th class="row_header"><div>'+name+'</div></th>';
	    		if(AdvancedStsController.measuresMap[0])
	    			{
				        for ( var u = 0; u < AdvancedStsController.measuresMap[0].col-1; u++) 
				        {
				        	 contents+='<td class="row_header data datax vv ulzhifen">-</td>';
						}
	    			}
		        for ( var i = 0; i < AdvancedStsController.measuresMap.length; i++) 
		        {
		        	var q = AdvancedStsController.measuresMap[i][prop];
		        	if(!(s === false))
		        		{
		        			q = (q).formatMoney(2, s || "", ".", ",");
		        		}
		        	contents+='<td class="data ulz datax vv">'+q+'</td>';
				}
		        contents += "</tr>";
    		}
        return contents;
    ...
    post_process: function() {
        ...
        this.insertHorizontalValues();
        this.workspace.trigger('table:rendered', this);
    },
    insertHorizontalValues:function()
    {
    	this.renderHorizontal('Somat&oacute;rio(&Sigma;)','sumX');
    	this.renderHorizontal('M&eacute;dia (&#955;)','avgX');
    },
    renderHorizontal:function(name,prop,s)
    {
    	if(!($('.col.headerhighlight').last().parent().children('.col.headerhighlight').length >1) || AdvancedStsController.measuresMap.length == 0)return;
    	var parent = $('.col.headerhighlight').last().parent();
    	
    	this.insertOffSetHeader();
    	
    	parent.append('<th class="col row_header" style="text-align: center;" colspan="1" '+
    			+'title="'+name+'"><div rel="0:4">'+name+'</div></th>');
    	
    	var trs = $('.data').not('.ulz,.ulzhifen').parent();
    	
    	var i = 0;
    	for ( var j = 0; j < AdvancedStsController.measuresMap[i][prop].length; j++) 
		{
    		var vlr = (AdvancedStsController.measuresMap[i][prop][j])
    		.formatMoney(2, "" || "", ".", ",");
    		$(trs[j]).append($('<td class="data datax vv">'+vlr+'</td>'));
		}
    	this.renderULZ();
    },
    insertOffSetHeader:function()
    {
    	var parent =  $('.col.headerhighlight').parent();
    	
    	for ( var j = 0; j < parent.length-1; j++) 
    	{
    		$(parent[j]).append('<th class="col row_header" style="text-align: center;" colspan="1" '+
        			+'title="-"><div rel="0:4">-</div></th>');
		}
    },
    renderULZ:function()
    {
    	var ulrParentList = $('.ulz').parent();
    	
    	for ( var i = 0; i < ulrParentList.length; i++) 
    	{
			var ulzList = $(ulrParentList[i]).find('.ulz').not('.ulzhifen');
			var vlr/*:String*/ =  sumSeriesHTML(ulzList);
			$(ulrParentList[i]).append('<td class="data datax vv">'+vlr+"</td>");
		}
    }
});
