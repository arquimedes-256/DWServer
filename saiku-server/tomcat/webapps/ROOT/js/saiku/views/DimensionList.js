...
var DimensionList = Backbone.View.extend({
    ...
    initialize: function(args) {
		...
		$('.sidebar').niceScroll();
		this.loadPatternProject();
		
    },
    loadPatternProject:function()
    {
    	/*
    	var dimesionList = $('.parent_dimension').children('a');
    	var blackList = '';
    	
    	for ( var i = 0; i < dimesionList.length; i++) 
    	{
			var parent= $(dimesionList[i]).next();
			var prefix = $(dimesionList[i]).attr('title').match(/\((.*?)\)/)[1];
			var prefixedItens = $('[title^="('+prefix+')"]');
			for ( var j = 1; j < prefixedItens.length; j++) 
			{
				var content = $(prefixedItens[j]).next().find('li');
				parent.append(content);
				$(prefixedItens[j]).parent('li').hide();
			}
			$(dimesionList[i]).html(prefix);
		}
    	console.log('loaded');
    	*/
    },
    ...
    select: function(event) {
        ...
        var h = $target.parents('li').find('ul').height();
        var n = $target.parent().children('ul').children('li').length;
        if ($target.hasClass('root')) {
            ...
            console.log($target.parent().children('ul').children('li').length)
            if($target.hasClass('expand'))
            	{
            		$target.parents('li').find('ul').children('li').show();
            		TweenMax.to($target.parents('li').find('ul'),.2*n,
            				{css:{height:20 * n}});
            	}
            else
            	{
            		TweenMax.to($target.parents('li').find('ul'),.2*n,{css:{height:0},onComplete:function()
            			{
            				$target.parents('li').find('ul').children('li').hide();
            			}});
            	}
        }
