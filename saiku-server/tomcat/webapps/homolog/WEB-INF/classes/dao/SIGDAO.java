...
public class SIGDAO {
	...
	public ArrayList<Row> orcadorealizado_detalhado(Options options) throws SQLException
	{
		...
		while(rs.next())
		{
			...
			double orc = rs.getDouble("ORCADO");
			double orcnovo = rs.getDouble("VLR_ORCADONOVO");
			if(options.isOrcNovo)
			{
				row.setVlrOrcado(orcnovo);
				row.setVlrRealizado((row.getVlrRealizado()/11)*12);
			}
			else
				row.setVlrOrcado(orc);
