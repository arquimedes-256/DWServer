...
public class DefaultBus extends HttpServlet {
	...
	private void callDAO(HttpServletRequest req, HttpServletResponse resp) throws IOException
	{
		...
		try 
		{
			...
			options.idContaContabil = String.valueOf(req.getParameter("idContaContabil"));
			options.isOrcNovo = Boolean.valueOf(req.getParameter("orcNovo"));
			...
			if((jsonOutput = RowsCache.getInstance().get(options)) == null)
			{
				...
				{
					output = new RowsFactory().buildTotal((ArrayList<Row>) output,false);
				}
				
				if(options.metodo.equals("centro_de_custo_mtz"))
				{
					if(output != null)
						output = new RowsFactory().buildTotal((ArrayList<Row>) output,true);
				}
				if(output != null)
