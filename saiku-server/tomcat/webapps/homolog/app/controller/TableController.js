...
//centro_de_custo_mtz
	...
	$scope.hojeMes = $scope.mesAtual = 11//new Date().getMonth();
	...
	function refreshRel(rel,options)
	{
		...
		if(options && options.orcNovo)
		{
		 	$scope.mesAtual = locMesAtual = 1;
		 	$scope.mesFinal = locMesFinal = 11;
		 	
		 	$scope.mesAtualLabelModal = "Realizado 2014 ate Novembro vs Orcado 2015";
		}
		else
		{
			if($scope.mesAtual != $scope.mesFinal)
			{
				...
				$scope.mesAtualLabelModal = $scope.meses[$scope.mesAtual];
		}
		 var params = {
			mes:locMesAtual,
			ano:$scope.anoAtual,
