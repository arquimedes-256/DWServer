function TableController() {
    ...
    function _setValueOnInput(h, m, refInput) {
        ...
        if (isNaN(H)) H = 0;
        inputReal.val(new String(H).replace('.', ','));
    ...
    function setRealToMMSS() {
        $('.inpt-hmcp,.inpt-hmca').each(function() {
            ...
            if (MMSS == "NaN:NaN") MMSS = "";
            if (parseInt(val) == 0) MMSS = "";
            hhmmInput.val(MMSS);
            console.log(v, val);
        })
    }
    ...
    function calculaLitroHoraDoInput(inpt) {
        ...
        if (isNaN(x)) x = "-"
        else x = parseFloat(Math.round(x * 100) / 100).toFixed(2);
        lhInput.val(x);
    }
    ...
    function login() {
        if (jQuery.browser.name != 'Chrome' && jQuery.browser.name != 'Firefox') {
    ...
    function _setRowsFromMounth() {
        ...
        for (var i = 1; i < 32; i++) {
            ...
            for (var k = 0; k < comandantes.length; k++) comandantesLista += '<li><a>' + comandantes[k].nome + '</a></li>';
            if (parseFloat(z.match(/^..\/(..)/)[1]) != parseFloat(idMes)) break;
            var tr = '<tr class="rowValue">' + '<td><input type="text" class="input-small inpt-data" placeholder="Data" id="' + (zeroFill(i) + "-" + zeroFill(idMes) + "-" + idAno) + '" value="' + (zeroFill(i) + "/" + zeroFill(idMes) + "/" + idAno) + '" disabled="true"></td>' + '<td class="control-group"><input type="text" class="input-mini inpt-saldo" disabled="true"></td>' + '<td class="control-group"><input type="text" class="input-mini inpt-compra zz" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Compra"></td>' + '<td class="control-group"><input type="text" class="input-mini inpt-acd zz" data-toggle="tooltip" data-placement="bottom" title=""  data-original-title="Acerto/Perda/Descarte"></td>' + '<td class="control-group"><input type="text" class="input-mini inpt-recebido zz" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Tranferência Recebido"></td>' + '<td class="control-group"><input type="text" class="input-mini inpt-fornecido zz" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Tranferência Fornecido"></td>' + '<td class="control-group"><input type="text" class="input-mini inpt-cmcp zz" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Consumo MCP"></td>' + '<td class="control-group"><input type="text" class="input-mini inpt-cmca zz" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Consumo MCA"></td>' + '<td class="control-group"><input type="text" class="input-mini inpt-hmcp zz" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Horas MCP"><input type="text" class="input-mini hrtype hmcp" value="..."/></td>' + '<td class="control-group"><input type="text" class="input-mini inpt-hmca zz" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Horas MCA"><input type="text" class="input-mini hrtype hmca" value="..."/></td>' + '<td class="control-group"><input type="text" class="input-mini inpt-lh zz" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Litro/H"></td>' +
                /**
...
*/
    ...
    function buildJson() {
        ...
        var rows = $('.rowValue');
        var jsonObject = [];
        var urlSTR = APP_CONFIG.URL + "insertRegistros&show_error&idFilial=" + idFilial + '&idRebocador=' + idRebocador + '&idAno=' + idAno + '&idMes=' + (idMes + "".length == 1 ? "0" + idMes : idMes);
        rows.each(function() {
            jsonObject.push({
                    ...
                    filial: $(this).find('.filial').val(),
                    rebocador: $(this).find('.rebocador').val() || idRebocador
                } //TODO
            );
        });
        ...
        $('.salvarRegistros').fadeOut();
        if (enableSave) {
            $.ajax(urlSTR + "&_=" + Math.random() + "e" + Math.random(), {
                type: "POST",
                data: {
                    json: jsonString
                },
    ...
    function _loadRowsFromDB(l) {
        ...
        for (var i = 0; i < rows.length; i++) { //CONS_MCP
            ...
            var SIGLA_FILIAL = $(rows[i]).find('SIGLA_FILIAL').html();
            var ID_REBOCADOR = $(rows[i]).find('ID_REBOCADOR').html();
            if (_getFromCDATA(SALDO, null, true)) tr.find('.inpt-saldo').val(_getFromCDATA(SALDO));
            if (_getFromCDATA(COMPRA, null, true)) tr.find('.inpt-compra').val(_getFromCDATA(COMPRA));
            if (_getFromCDATA(ACERTO_PERDA_DESCARTE, null, true)) tr.find('.inpt-acd').val(_getFromCDATA(ACERTO_PERDA_DESCARTE));
            if (_getFromCDATA(TRANS_RECEB, null, true)) tr.find('.inpt-recebido').val(_getFromCDATA(TRANS_RECEB));
            if (_getFromCDATA(TRANS_FORNEC, null, true)) tr.find('.inpt-fornecido').val(_getFromCDATA(TRANS_FORNEC));
            if (_getFromCDATA(CONS_MCP, null, true)) tr.find('.inpt-cmcp').val(_getFromCDATA(CONS_MCP));
            if (_getFromCDATA(CONS_MCA, null, true)) tr.find('.inpt-cmca').val(_getFromCDATA(CONS_MCA));
            if (_getFromCDATA(HR_MCP, null, true)) tr.find('.inpt-hmcp').val(_getFromCDATA(HR_MCP));
            if (_getFromCDATA(HR_MCA, null, true)) tr.find('.inpt-hmca').val(_getFromCDATA(HR_MCA));
            if (_getFromCDATA(SALDO_FINAL, null, true)) tr.find('.inpt-saldofinal').val(_getFromCDATA(SALDO_FINAL));
            ...
            tr.find('.filial').val(_getFromCDATA(SIGLA_FILIAL));
			var x = _getFromCDATA(ID_REBOCADOR);
			if(x == "" || !x) {
				x = idRebocador;
			}
            tr.find('.rebocador').val(x);
            /**
            ...
            */
            if (idFilial == 77 || (((dataDaLinha < dataAtual) || (dataDaLinha > dataAtual)) || parseInt(idMes) != _dataAtual.getMonth() + 1)) {
                if (tr.find(allInput).attr('prevent-disabled') === undefined) {
                    var dataDaLinhaDT = new Date(idAno + "-" + idMes + "-" + (dataDaLinha-1));
                    /** 
s
				...
				**/
                   console.log(exceptions);
                    if (!(exceptions[idFilial] && (dataDaLinhaDT >= new Date(exceptions[idFilial].dtInicio+1) && dataDaLinhaDT <= exceptions[idFilial].dtFim))) {
                        tr.find('input').attr('disabled', true);
                        if (!removeu) $('.salvarRegistros button').attr('disabled', true);
                    } else {
                        removeu = true;
                    }
                }
        ...
        if (idMes >= config["Bloquear a partir do mes"]) {
            $('input').attr('disabled', '');
        } else $('.salvarRegistros button').removeAttr('disabled');
        //BLOCK ( em agosto mudar para 8)
		
		if(config["Forcar abrir dia"] || (exceptions[idFilial] && exceptions[idFilial]["Forcar abrir dia"])) {
		
			if(exceptions[idFilial] && exceptions[idFilial]["Forcar abrir dia"])
				config["Forcar abrir dia"] = exceptions[idFilial]["Forcar abrir dia"];
				
			$('input[id="'+config["Forcar abrir dia"]+'"]').parent()
				.parent().find(allInput.replace(".inpt-saldo,","")+",.hrtype").removeAttr('disabled', '');
			$('input[id="'+config["Forcar abrir dia"]+'"]').parent()
				.parent().find(allInput.replace(".inpt-saldo,","")+",.hrtype").removeAttr('disabled', '');
    ...
    function getRebocadoresByFilial() {
        _getIt("com_rebocadoresByFilial&idFilial=" + idFilial, 'rHTML', function() {
            _setRebInDropDown();
        });
    }
    ...
    function getComandantesByFilial() {
        _getIt("com_comandantesByFilial&idFilial=" + loginUsuario, 'comandantesHTML', function() {
            _setComandantesInScope();
        });
        _getIt("com_maritimosByFilial&idFilial=" + loginUsuario, 'maritimosHTML', function() {
    ...
    function _setMaritimosInScope() {
        ...
        for (var i = 0; i < maritimos.length - 1; i++) {
            ...
            tr += "<td>" + m.nome + "</td>";
            tr += "<td>" + m.funcao + "</td>";
            tr += "<td>" + '<button onclick="tableController.swapStatusMaritimo(' + i + ')" class="btn ' + btnType + '">' + (m.situacao === 'A' ? 'Esperar!' : 'Comandar!')
            '</button>' + "</td>";
            tr += "</tr>";
    ...
    function swapStatusMaritimo(i) {
        ...
        _getIt("com_swapStatusMaritmo&situacao=" + situacao + "&matricula=" + matricula, 'none', function() {
            getComandantesByFilial();
            buildJson();
        });
    }
    ...
    var blockIsFirstDay = function() {
        ...
        if (checkIfFirstFithBusinessDay(dtAtual.getDate(), dtAtual.getMonth() + 1, dtAtual.getYear()) && idMes < (dtAtual.getMonth() + 1) && dtAtual.getHours() >= 11) {
